  ...
  return <div ref={surface} className="flex min-h-0 w-full shrink-0 flex-col">
    ...
    <div className="relative min-h-0 flex-1"><div ref={list} data-video-scroll className="h-full overflow-y-auto overscroll-none px-4 pb-24 pt-4" aria-busy={query.isFetching}>
      {query.isPending ? <p role="status" className="py-16 text-center text-sm text-mute">영상을 불러오는 중...</p> : query.isError ? <div role="alert" className="py-10 text-sm text-[#A93226]">영상을 불러오지 못했습니다.<button onClick={() => query.refetch()} className={`${button} mt-3`}>다시 시도</button></div> : !videos.length ? <p role="status" className="py-16 text-center text-sm text-mute">선택한 조건에 맞는 영상이 없습니다.</p> : <ul aria-label="영상 목록" className="space-y-3">{videos.map(video => <li key={video.videoId} className="rounded-[3px] border border-hairline bg-white p-3">
        <a href={video.videoType === 'shorts' ? `https://www.youtube.com/shorts/${video.videoId}` : `https://www.youtube.com/watch?v=${video.videoId}`} target="_blank" rel="noopener noreferrer" aria-label={`${decodeHtmlEntities(video.title)} · YouTube에서 보기`} className="flex gap-3"><img src={`https://i.ytimg.com/vi/${video.videoId}/mqdefault.jpg`} alt="" loading="lazy" className={`shrink-0 border border-hairline object-cover ${video.videoType === 'shorts' ? 'h-24 w-16' : 'h-[63px] w-28'}`} /><div className="min-w-0 flex-1"><h2 className="line-clamp-2 break-words text-sm font-bold leading-relaxed">{decodeHtmlEntities(video.title)}</h2><p className="mt-1 break-words text-xs text-mute">{video.channelName}</p><p className="mt-1 text-xs text-mute">{video.publishedAt?.slice(0, 10).replaceAll('-', '. ')}</p><span className="mt-2 inline-block rounded bg-green-soft px-1.5 py-1 text-xs font-bold text-green-deep">{video.videoType === 'shorts' ? 'SHORTS' : categoryName(video.category)}</span></div></a>
        <div className="mt-3 grid grid-cols-[1fr_72px] gap-2"><button onClick={() => setEditor(video)} className="min-h-11 bg-ink text-sm font-bold text-white">수정</button><button onClick={() => { setError(''); setDeleting(video); }} className={`${button} bg-white`}>삭제</button></div>
      </li>)}</ul>}
