...
import BotAddMenu from './BotAddMenu';
...
function BotsContent() {
  ...
  return <>
    ...
    <BotAddMenu disabled={Boolean(busy)} hidden={Boolean(editing || deleting || draft)} onSelect={type => { hideToast(); setEditing({ type, db_id: null }); }} />
    {[['youtube', YouTubeBotDialog], ['x', XBotDialog], ['festival', FestivalBotDialog]].map(([type, Editor]) => <Editor key={type} mobile isOpen={editing?.type === type} botId={editing?.type === type ? editing.db_id : null} onClose={() => setEditing(null)} onSuccess={() => { showSuccess(editing?.db_id ? '봇을 수정했습니다.' : '봇을 추가했습니다.'); client.invalidateQueries({ queryKey: ['admin', 'logs'] }); client.invalidateQueries({ queryKey: ['admin', 'mobile'] }); }} />)}
