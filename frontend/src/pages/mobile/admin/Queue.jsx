...
import { Search, X, Inbox, AlertCircle } from 'lucide-react';
...
function QueueList() {
  ...
  return <>
    ...
    <div className="mt-6 flex min-h-12 items-center gap-2 border border-hairline rounded-[4px] bg-white pl-3 focus-within:border-ink">
      ...
      {filtered.length === 0 ? <p className="py-16 text-center text-sm text-mute">검색 결과가 없습니다.</p> : <ul aria-label="검토 대기 일정" className="mt-5 space-y-4">
        {filtered.map(item => {
          ...
          <div className="flex items-start gap-3">
            <div className="flex min-h-[70px] w-[60px] shrink-0 flex-col items-center justify-center rounded-[4px] bg-[#F5F5F3] px-1 py-2 text-center" aria-label={date?.full || '날짜 미정'}>
            ...
            <div className="min-w-0 flex-1 py-1">
              ...
              <p className="mt-2 break-words text-[13px] leading-relaxed text-mute">{item.time || '시간 미정'}{!item.date || !item.time ? ' · 정보 보완 필요' : ''}</p>
            </div>
          </div>
          ...
          <div className="mt-3 grid grid-cols-[2fr_3fr] gap-2">
            <button type="button" disabled title="목록 확인 단계입니다" className="min-h-11 rounded-[2px] border border-ink bg-white px-3 text-[14px] font-medium text-ink disabled:cursor-not-allowed">무시</button>
