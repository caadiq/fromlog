...
export default function LinkEditor({ item, busy, saveError, onClose, onSave }) {
  ...
  return createPortal(<AnimatedDialog open={Boolean(item)} ref={dialog} className="mobile-queue-review" aria-label={item?.id ? '링크 수정' : '링크 추가'} onCancel={event => { event.preventDefault(); onClose(); }}><div className="flex h-full flex-col bg-paper text-ink">
  ...
  </div></AnimatedDialog>, document.body);
}
