...
1-1. 한 줄에 날짜 후보가 여럿이면("8/21 or 28", "8/21 또는 28", "8/21~28 중") 아직 확정 전이라는 뜻이다. 항목을 후보 수만큼 쪼개지 말고 **한 건**으로 만들되 date는 빈 문자열, description에 "8/21 또는 8/28"처럼 후보를 적어라. 단 "1일차/2일차", "EP.7/EP.8"처럼 실제로 여러 번 열리는 일정은 각각 별개 항목이다.
1-2. 멤버 생일 그 자체("하영 생일", "9/29 송하영 생일")는 **항목으로 만들지 마라** — 사이트가 멤버 정보로 매년 자동 표시한다. 다만 생일카페·생일 광고·서포트처럼 그날 따로 열리는 행사는 별개 일정이니 포함하라.
