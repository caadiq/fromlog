...
- 홈: 실제 대기 건수(`/admin/pending/count`), 전체 등록 일정(`/stats`), 최근 활동 4건(`/admin/logs`)과 새로고침·빈 상태·조회 오류 표시.
- 하단 네비게이션 없이 상단 메뉴 버튼으로 사이드 메뉴를 연다. Escape·배경·브라우저 뒤로가기로 닫을 수 있고, 로그아웃 시 관리자 쿼리 캐시를 제거한다.
