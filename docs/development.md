...
- `/admin/schedule/queue`에 모바일 전용 조회 목록을 추가했다. 기존 `GET /admin/pending` 응답을 사용하며 등록·무시·수정 요청은 하지 않는다. 해당 액션과 등록 폼은 후속 작업이다.
- 카테고리 탭·필터·배지 없이 날짜·시간·제목·장소·멤버·설명을 카드로 표시한다. 중복 의심, 최신 원문에서 사라짐, 등록 미완료 표시는 유지한다.
